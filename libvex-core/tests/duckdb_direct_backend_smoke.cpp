...
int main() {
    ...
    cfg.m = 8;
    cfg.metadata_size = 0;
    cfg.index_name = "direct_mock_smoke";
    ...
    graph.AddPoint(200, &data[0], cfg.dimension);
    graph.AddPoint(201, &data[4], cfg.dimension);
    graph.AddPoint(202, &data[8], cfg.dimension);
    ...
    if (rows.empty()) {
        std::cerr << "duckdb direct backend search empty" << std::endl;
        return 1;
    }
