...
class PersistentPGBinding final : public vex::PGLowLevelBinding {
public:
    PersistentPGBinding(std::shared_ptr<vex::PGLowLevelBindingMock> delegate,
...
private:
    std::shared_ptr<vex::PGLowLevelBindingMock> delegate_;
...
int main() {
    auto delegate = std::make_shared<vex::PGLowLevelBindingMock>(4, 8, 0);
    auto state = std::make_shared<SharedGraphState>();

    auto binding_build = std::make_shared<PersistentPGBinding>(delegate, state);
    vex::PGNodeStoreConfig build_cfg{};
    build_cfg.dimension = 4;
    build_cfg.m = 8;
    build_cfg.metadata_size = 0;
    build_cfg.index_name = "pg_graph_state_roundtrip_build";
    build_cfg.low_level_binding = binding_build;

    vex::PGNodeStore build_store(build_cfg);
    ...
    build_graph.AddPoint(2000, &data[0], build_cfg.dimension);
    build_graph.AddPoint(2001, &data[4], build_cfg.dimension);
    build_graph.AddPoint(2002, &data[8], build_cfg.dimension);

    if (!vex::StoreGraphStateToBinding(*binding_build, build_graph)) {
        std::cerr << "pg state store failed" << std::endl;
        return 1;
    }

    auto binding_reload = std::make_shared<PersistentPGBinding>(delegate, state);
    vex::PGNodeStoreConfig reload_cfg = build_cfg;
    reload_cfg.index_name = "pg_graph_state_roundtrip_reload";
    reload_cfg.low_level_binding = binding_reload;
    vex::PGNodeStore reload_store(reload_cfg);
    ...
    if (rows.empty() || rows[0] != 2001) {
        std::cerr << "pg graph state roundtrip search mismatch" << std::endl;
        return 4;
    }
