...
#include "graph_index/core_memory_build_runtime.hpp"
...
private:
...
#ifdef PG_VEXDB_ENABLE_LIBVEX_CORE_BRIDGE
    std::unique_ptr<pgvexdb::CoreMemoryBuildRuntime> core_memory_runtime;
#endif
    ...
    {
        core_memory_runtime = std::make_unique<pgvexdb::CoreMemoryBuildRuntime>();
        if (!core_memory_runtime->Init(dimension, m, ef_construction, pgvexdb::ToCoreBridgeMetric(metric))) {
    ...
    void flush_graph(Relation index)
    {
        ...
        build_state = BuildState::DISK;
        if (core_memory_runtime && core_memory_runtime->node_store && core_memory_runtime->core_graph) {
            pgvexdb::CoreMemoryBuildFlushInput flush_input{
                *core_memory_runtime->node_store,
                *core_memory_runtime->core_graph,
                core_memory_runtime->heap_tids_by_node_id
            };
            pgvexdb::FlushCoreMemoryBuildToDisk(index, metabuf, metap, dimension, m, vector_size, flush_input);
            return;
        }
