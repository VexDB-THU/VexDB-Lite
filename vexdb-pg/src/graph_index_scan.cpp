...
bool graph_index_gettuple_internal(IndexScanDesc scan, void *in_so, BlockNumber metablkno, size_t ef, float *dist_out)
{
    ...
    if (so->first) {
        ...
        auto visitor = [&](auto &store) -> void {
            auto run_search = [&](auto &distancer) {
                PointExtensionContext ctx(index, GRAPH_INDEX_PS_BLKNO, false);
                pgvexdb::CoreBridgeSearchResult core_res;
                if (pgvexdb::TrySearchViaCoreBridge(store, index, metap, ctx, query, ef, core_res)) {
                    so->tid_count = static_cast<int>(core_res.heaptids.size());
                    if (so->tid_count > 0) {
                        ...
                        memcpy(so->heaptids, core_res.heaptids.data(),
                               so->tid_count * sizeof(ItemPointerData));
                        memcpy(so->dists, core_res.dists.data(),
                               so->tid_count * sizeof(float));
                    }
                    so->first = false;
                    so->has_more_data = core_res.has_more_data;
                    ctx.destroy();
                    return;
                }
            ...
            if constexpr (std::decay_t<decltype(store)>::clustered) {
                DispatchRunner<true,
                    ...
                    >, DispatcherMode::NO_QUANT>::call(metap, run_search);
            } else {
                DispatchRunner<true,
                    MetricList<Metric::L2, Metric::INNER_PRODUCT, Metric::FAST_COSINE>,
                    DistPrecisionTypeList<
                        DistPrecisionType::FLOAT,
                        DistPrecisionType::HALF
                    >, DispatcherMode::DEFAULT>::call(metap, run_search);
            }
        };
