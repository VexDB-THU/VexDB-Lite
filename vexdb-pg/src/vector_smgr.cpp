 ...
 * Adapted for PostgreSQL: VECTOR_FORKNUM = VISIBILITYMAP_FORKNUM
 */
...
#define VEC_SEG_ARRAY(reln) ((MdfdVec *)(reln)->md_seg_fds[VISIBILITYMAP_FORKNUM])

/*
 * Get segment path for visibility map fork
 */
static char *
vec_segment_path(SMgrRelation reln, BlockNumber segno)
{
    (void)segno;
    return relpath(reln->smgr_rlocator, VISIBILITYMAP_FORKNUM);
}
...
vec_openseg(SMgrRelation reln, BlockNumber segno, int oflags)
{
    File fd;
    char *path;
    char fullpath[MAXPGPATH + 1];
    
    path = relpath(reln->smgr_rlocator, VISIBILITYMAP_FORKNUM);
    
    if (segno > 0)
        snprintf(fullpath, sizeof(fullpath), "%s.%u", path, segno);
    else
...
vec_getseg(SMgrRelation reln, BlockNumber blkno, int behavior)
{
    ...
    if (targetseg < (BlockNumber)reln->md_num_open_segs[VISIBILITYMAP_FORKNUM]) {
    ...
    if (behavior & VEC_EXTENSION_RETURN_NULL) {
        ...
        if (reln->md_num_open_segs[VISIBILITYMAP_FORKNUM] == 0) {
            reln->md_seg_fds[VISIBILITYMAP_FORKNUM] = 
                (struct _MdfdVec *)MemoryContextAlloc(TopMemoryContext, 
                                                       sizeof(MdfdVec) * (targetseg + 1));
        } else if (targetseg >= (BlockNumber)reln->md_num_open_segs[VISIBILITYMAP_FORKNUM]) {
            reln->md_seg_fds[VISIBILITYMAP_FORKNUM] = 
                (struct _MdfdVec *)repalloc(reln->md_seg_fds[VISIBILITYMAP_FORKNUM],
                                             sizeof(MdfdVec) * (targetseg + 1));
        }
        ...
        for (BlockNumber i = reln->md_num_open_segs[VISIBILITYMAP_FORKNUM]; i < targetseg; i++) {
        ...
        reln->md_num_open_segs[VISIBILITYMAP_FORKNUM] = targetseg + 1;
    ...
    if (behavior & VEC_EXTENSION_CREATE) {
        ...
        for (BlockNumber segno = reln->md_num_open_segs[VISIBILITYMAP_FORKNUM]; 
            ...
            if (reln->md_num_open_segs[VISIBILITYMAP_FORKNUM] == 0) {
                reln->md_seg_fds[VISIBILITYMAP_FORKNUM] = 
                    (struct _MdfdVec *)MemoryContextAlloc(TopMemoryContext, 
                                                           sizeof(MdfdVec) * (targetseg + 1));
            } else if (segno >= (BlockNumber)reln->md_num_open_segs[VISIBILITYMAP_FORKNUM]) {
                reln->md_seg_fds[VISIBILITYMAP_FORKNUM] = 
                    (struct _MdfdVec *)repalloc(reln->md_seg_fds[VISIBILITYMAP_FORKNUM],
                                                 sizeof(MdfdVec) * (targetseg + 1));
            }
            ...
            reln->md_num_open_segs[VISIBILITYMAP_FORKNUM] = segno + 1;
        }
...
retry:
    cur_pool.locmap->cvisit(sig, [&](const auto &x) {
        if (!x.second.valid()) {
            return;
        }
    ...
    if (!cur_pool.locmap->try_emplace(sig, new_loc)) {
...
/* Initialization */
bool enable_vec_buffer_manager() { return pg_vexdb_is_preloaded() && vecbuf_shared_state && vecbuf_shared_state->enable_buffer_manager; }

void init_vector_smgr()
{
    if (!pg_vexdb_is_preloaded()) {
        return;
    }
...
void vec_invalidate_buffer_cache(Oid relNode, size_t loc, size_t elem_size)
{
    ...
    cur_pool.locmap->erase_if(sig, [&cur_pool](auto &x) {
        if (x.second.empty()) {
            return true;
        }
        auto loc = x.second;
                           ...
                           char *buffer, VecStorageType vec_storage_type)
{
    ...
    while (read_bytes < nbytes) {
        ...
        /* Check buffer alignment for direct I/O */
        uintptr_t buf_addr = (uintptr_t)(buffer + read_bytes);
        bool buf_aligned = (buf_addr % PG_IO_ALIGN_SIZE) == 0;
        bool offset_aligned = (seg_offset % PG_IO_ALIGN_SIZE) == 0;
        bool size_aligned = (chunk % PG_IO_ALIGN_SIZE) == 0;
        
        ssize_t nread;
        
        if (buf_aligned && offset_aligned && size_aligned && chunk >= BLCKSZ) {
            /* Direct read into aligned buffer */
            nread = FileRead(seg->mdfd_vfd, buffer + read_bytes, chunk, seg_offset,
                ...
                const char *buffer, bool skip_fsync, VecStorageType vec_storage_type)
{
    ...
    while (written < nbytes) {
        ...
        if (seg == NULL) {
            char *path = vec_segment_path(reln, blocknum / RELSEG_SIZE);
            ereport(ERROR,
                    (errcode_for_file_access(),
                     errmsg("could not open vector file \"%s\": %m", path)));
        }
        ...
        /* Check buffer alignment for direct I/O */
        uintptr_t buf_addr = (uintptr_t)(buffer + written);
        bool buf_aligned = (buf_addr % PG_IO_ALIGN_SIZE) == 0;
        bool offset_aligned = (seg_offset % PG_IO_ALIGN_SIZE) == 0;
        bool size_aligned = (chunk % PG_IO_ALIGN_SIZE) == 0;
        ...
        if (buf_aligned && offset_aligned && size_aligned && chunk >= BLCKSZ) {
            /* Direct write from aligned buffer */
            nwritten = FileWrite(seg->mdfd_vfd, buffer + written, chunk, seg_offset,
            ...
            if (aligned_offset < seg_offset || aligned_size > chunk) {
                ssize_t nread = FileRead(seg->mdfd_vfd, tmp, aligned_size, aligned_offset,
                                          WAIT_EVENT_DATA_FILE_READ);
                if (nread < 0)
                    nread = 0;
                if ((size_t)nread < aligned_size)
                    memset(tmp + nread, 0, aligned_size - nread);
            } else {
                memset(tmp, 0, aligned_size);
            }
            
            /* Copy data into aligned buffer */
            size_t copy_offset = seg_offset - aligned_offset;
            memcpy(tmp + copy_offset, buffer + written, chunk);
            
            /* Write aligned chunk */
            nwritten = FileWrite(seg->mdfd_vfd, tmp, aligned_size, aligned_offset,
                                  WAIT_EVENT_DATA_FILE_WRITE);
        ...
        if (nwritten != (ssize_t)chunk) {
            ...
            ereport(ERROR,
                    (errcode_for_file_access(),
                     errmsg("could not write to vector file \"%s\": wrote %zd of %zu bytes",
                            path, nwritten, chunk)));
        }
...
void create_vec_data(Relation rel, bool need_wal)
{
    ...
    /* Create visibility map fork for vector storage */
