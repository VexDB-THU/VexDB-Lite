...
/// NFSv3 COMMIT. The VFS implementation decides how to make the requested
/// range durable. The write verifier must match WRITE responses from the same
/// server lifetime.
pub async fn nfsproc3_commit(
    ...
    args.deserialize(input)?;
    debug!("nfsproc3_commit({:?}, {:?}, {:?})", xid, args.offset, args.count);
    ...
    let pre_attr = match context.vfs.getattr(id).await {
        Ok(value) => nfs::pre_op_attr::attributes(nfs::wcc_attr {
            size: value.size,
            mtime: value.mtime,
            ctime: value.ctime,
        }),
    ...
    match context.vfs.commit(id, args.offset, args.count).await {
        Ok(value) => {
            ...
            COMMIT3resok {
                file_wcc: nfs::wcc_data {
                    before: pre_attr,
                    after: nfs::post_op_attr::attributes(value),
                },
                verf: context.vfs.serverid(),
            }
            .serialize(output)?;
        ...
        Err(status) => {
            let post_attr = match context.vfs.getattr(id).await {
                Ok(value) => nfs::post_op_attr::attributes(value),
                Err(_) => nfs::post_op_attr::Void,
            };
            make_success_reply(xid).serialize(output)?;
            status.serialize(output)?;
...
pub async fn nfsproc3_link(
    ...
    let fileid = match context.vfs.fh_to_id(&args.file) {
        Ok(value) => value,
        Err(status) => {
            make_success_reply(xid).serialize(output)?;
            status.serialize(output)?;
    ...
    match result {
        Ok(value) => {
            make_success_reply(xid).serialize(output)?;
            nfs::nfsstat3::NFS3_OK.serialize(output)?;
            nfs::post_op_attr::attributes(value).serialize(output)?;
