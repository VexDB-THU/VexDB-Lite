...
| Parallel build | ✅ Done | Memory and disk build both work with parallel workers |
...
Parallel build uses PostgreSQL's DSM (Dynamic Shared Memory) via `shm_toc` for shared state. All workers share the same graph in DSM memory during memory build, or use independent DiskStore instances during disk build.

### Key Components

**1. AllocatorState (unified allocator for single-thread and parallel):**
```cpp
struct AllocatorState {
    std::atomic<size_t> offset{0};
    char *base{nullptr};
    Size total_size{0};
    MemoryContext ctx{nullptr};  /* For single-threaded palloc fallback */
};
```

**2. GraphIndexShared (DSM-allocated shared state):**
```cpp
struct GraphIndexShared {
    Oid heaprelid, indexrelid;
    BlockNumber metablkno;
    bool isconcurrent;
    
    /* Index parameters */
    uint16 dimension, m, ef_construction;
    Metric metric;
    ...
    LWLock allocator_lock, flush_lock, entry_lock, entry_waitlock;
    std::atomic<uint32_t> num_vectors, num_uppers;
...
#define PARALLEL_KEY_GRAPH_INDEX_AREA     UINT64CONST(0x76580002)
#define PARALLEL_KEY_QUERY_TEXT           UINT64CONST(0x76580003)
```

### Memory Build (Parallel)

- `MemStore` uses bump allocator with atomic offset from DSM
- Workers share graph via DSM - all inserts go to shared memory
- Per-element `RWBitLock` for concurrent access
- `entry_lock`/`entry_waitlock` for entry point coordination
- Leader flushes to disk after all workers complete

### Disk Build (Parallel)

- Each worker has own `DiskStore` instance
- Uses buffer locks for metadata updates
- `base_layer.append()` / `upper_layer.append()` are atomic
- Vector writes via `vec_write()` are independent

### Key Design Decisions

1. **Unified Allocator** - Same `AllocatorState` works for both single-thread (palloc) and parallel (DSM)
2. **Bump Allocation** - Atomic fetch_add for thread-safe allocation without locks
3. **Raw Pointers** - Trust PostgreSQL's DSM address guarantee (same address in all workers)
4. **RWBitLock** - Works across processes (uses atomics internally)

### Files Modified

| File | Changes |
|------|---------|
| `src/graph_index_build.cpp` | `GraphIndexShared`, `GraphIndexLeader`, `begin_parallel()`, `end_parallel()`, `scan_and_insert()`, worker main |
| `include/graph_index/graph_index_storage.h` | `AllocatorState`, `bump_alloc()`, unified `MemPool`/`MemStore` constructor |
| `include/graph_index/graph_index.h` | `PARALLEL_KEY_*` constants |

### Temp Table Handling

Parallel build is disabled for temp tables:
```cpp
...
3. **Performance testing** - Large datasets (100K+ vectors)
