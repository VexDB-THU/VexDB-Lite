...
/* Simple allocator state - works for both single-thread (palloc) and parallel (DSM) */
...
/* Default allocator using atomic bump allocation */
inline void* bump_alloc(size_t size, void *state) {
    AllocatorState *s = (AllocatorState *)state;
    Size aligned = MAXALIGN(size);
    size_t old = s->offset.fetch_add(aligned, std::memory_order_relaxed);
    if (old + aligned > s->total_size) {
        elog(ERROR, "Allocator out of memory");
    }
    return s->base + old;
}

/* Fallback allocator using palloc (for small memory cases) */
inline void* palloc_fallback(size_t size, void *state) {
    AllocatorState *s = (AllocatorState *)state;
    return MemoryContextAlloc(s->ctx, size);
}
...
public:
    ...
    class MemPool {
        ...
        /* Unified allocator - works for both single-thread and parallel */
    ...
    public:
        ...
        /* Unified constructor using bump allocator */
        MemPool(uint32 store_esize, uint32 target_size_mb, AllocatorState *state)
              ...
              alloc_state(state)
        {
            ...
            char *aligned_ptr = (char *)bump_alloc(chunk_size, alloc_state);
    ...
    MemStore(uint_fast16_t dim, uint_fast16_t m, uint_fast32_t vec_size, size_t vectorpool_initsize,
             size_t pointpool_initsize, AllocatorState *alloc_state)
        : vector_pool(vec_size, vectorpool_initsize, alloc_state),
          basepoint_pool(get_base_point_size(m), base_target_size_mb(pointpool_initsize, m), alloc_state),
          upperpoint_pool(get_upper_point_size(m), upper_target_size_mb(pointpool_initsize, m), alloc_state),
          dim(dim),
          m(m),
          vec_size(vec_size),
          num_vectors(0),
...
public:
        ...
        IndexBulkDeleteResult *stats, IndexBulkDeleteCallback callback, void *callback_state)
    {
        ...
        size_t start_idx = 0;
        const auto collect_deleted_tids = [&](GraphIndexPoint *elem) -> bool {
            vacuum_delay_point(false);
            bool is_dirty = false;
            uint32 num_removed = elem->vacuum_tids(
    ...
    void mark_deleted(size_t basepoint_num, size_t upperpoint_num)
    {
        ...
        const auto mark_elem = [&](GraphIndexPoint *elem) -> bool {
            /*
             * skip:
             * 1. live elements
            ...
            elem->set_deleted();
            size_t id = elem_offset;
            vec_invalidate_buffer_cache(relNode, id, elem_size);
            xlog.log_invalidate_vector_cache(id, elem_size);
            recycled_ids.push_back((T)elem_offset);
            elem_offset++;
            return true;
        };
        elems.template apply<AccessorLockType::WriteLock>(mark_elem)(0, basepoint_num);
        ...
        upper_layer.template apply<AccessorLockType::WriteLock>(
            [&](GraphIndexDiskUpperPoint<T> *upperpoint) -> bool {
                if (!deleted_set.contains(upperpoint->id)) {
                    upper_offset++;
                    return false;
                }
                upperpoint->init(m);
                recycled_upper_indices.push_back((T)upper_offset);
                upper_offset++;
                return true;
            }
        )(0, upperpoint_num);
                              ...
                              BulkBuffer *bulkbuf, bool need_wal)
{
    ...
    if (GRAPH_INDEX_PAGE_GET_META(BufferGetPage(metabuf))->id_type == IdType::U32) {
        if (bulkbuf != NULL) {
            var.template emplace<DiskStore<uint32, true>>(index, heap, metabuf, bulkbuf, need_wal);
        } else {
            var.template emplace<DiskStore<uint32, false>>(index, heap, metabuf, nullptr, need_wal);
        }
    } else {
        if (bulkbuf != NULL) {
            var.template emplace<DiskStore<size_t, true>>(index, heap, metabuf, bulkbuf, need_wal);
        } else {
            var.template emplace<DiskStore<size_t, false>>(index, heap, metabuf, nullptr, need_wal);
        }
    }
}
