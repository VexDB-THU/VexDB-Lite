...
private:
	...
	void Clear();
	struct CoreBridgeContext {
		explicit CoreBridgeContext(const string &index_name) : storage(index_name) {
		}

		IndexStorageInfo storage;
		DuckDBCoreBindingConfig cfg{};
		std::shared_ptr<::vex::DuckDBNodeStore> store;
		::vex::DuckDBLowLevelBinding *binding = nullptr;
		::vex::AdapterGraphState graph_state{};
		std::unique_ptr<::vex::HNSWGraph> core_graph;
	};

	void BuildViaCoreBridgeOrThrow(DataChunk &chunk, Vector &row_ids);
	void BuildParallelViaCoreBridgeOrThrow(const std::vector<float> &all_vectors, const std::vector<row_t> &all_row_ids,
	                                      idx_t total_count, uint32_t dim, const std::vector<uint8_t> &all_metadata);
	void DeleteViaCoreBridgeOrThrow(Vector &row_identifiers, idx_t count);
	void SearchViaCoreBridgeOrThrow(const float *query_vec, idx_t k, int ef,
	                               std::vector<row_t> &out_row_ids, std::vector<float> &out_distances,
	                               idx_t brute_force_threshold);
	void PQSearchViaCoreBridgeOrThrow(const float *query_vec, idx_t k, int ef,
	                                 std::vector<row_t> &out_row_ids, std::vector<float> &out_distances,
	                                 idx_t brute_force_threshold);
	::vex::PQDistancerCore &EnsureCoreBridgePQDistancerOrThrow();
	void FilteredSearchViaCoreBridgeOrThrow(const float *query_vec, idx_t k, int ef,
	                                       std::vector<row_t> &out_row_ids, std::vector<float> &out_distances,
	                                       const vex::FilterPredicate &filter, idx_t brute_force_threshold);
	std::unique_ptr<CoreBridgeContext> CreateCoreBridgeContextOrThrow();
	void LoadCoreBridgeGraphOrThrow(CoreBridgeContext &ctx) const;
	bool SyncCoreBridgeGraphState(CoreBridgeContext &ctx);
	bool WriteCoreBridgeMetadata(CoreBridgeContext &ctx, uint32_t node_id,
	                            const uint8_t *metadata, const char *operation);
	void ExpandCoreBridgeResults(const std::vector<row_t> &core_row_ids, const std::vector<float> &core_distances,
	                            idx_t k, std::vector<row_t> &out_row_ids, std::vector<float> &out_distances) const;
