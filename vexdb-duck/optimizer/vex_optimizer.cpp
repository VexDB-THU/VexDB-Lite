...
static idx_t DebugTableIndexValue(const duckdb::TableIndex &table_index) {
	return table_index.index;
}

static idx_t DebugProjectionIndexValue(const duckdb::ProjectionIndex &projection_index) {
	return projection_index.GetIndexUnsafe();
}
#endif
...
static bool TryResolveDistanceOrder(Expression *order_expr, LogicalGet *get, LogicalProjection *proj,
...
#ifdef VEX_OPTIMIZER_DEBUG
		std::cerr << "[VEX]   colref: table_index=" << DebugTableIndexValue(colref.binding.table_index)
		          << " col_index=" << DebugProjectionIndexValue(colref.binding.column_index)
		          << " proj->table_index=" << DebugTableIndexValue(proj->table_index)
	...
	std::cerr << "[VEX]   func=" << info.func_expr->function.name
	          ...
	          << " get->table_index=" << DebugTableIndexValue(get->table_index) << std::endl;
	...
	for (int ci = 0; ci < 2; ci++) {
		...
		if (c->GetExpressionClass() == ExpressionClass::BOUND_COLUMN_REF) {
			...
			std::cerr << "[VEX]   child[" << ci << "] colref: table=" << DebugTableIndexValue(cr.binding.table_index)
			          << " col=" << DebugProjectionIndexValue(cr.binding.column_index) << std::endl;
			...
			if (cast.child->GetExpressionClass() == ExpressionClass::BOUND_COLUMN_REF) {
				...
				std::cerr << "[VEX]     inner colref: table=" << DebugTableIndexValue(cr.binding.table_index)
				          << " col=" << DebugProjectionIndexValue(cr.binding.column_index) << std::endl;
			}
		}
	}
...
static bool TryFindMatchingIndex(ClientContext &context, DataTable &storage,
...
#ifdef VEX_OPTIMIZER_DEBUG
	std::cerr << "[VEX] TryFindMatchingIndex: col_index=" << col_index
	          << " column_ids.size=" << column_ids.size()
	          << " physical_col_id=" << physical_col_id
	          << " func=" << func_name << std::endl;
#endif
	if (physical_col_id == DConstants::INVALID_INDEX) {
#ifdef VEX_OPTIMIZER_DEBUG
		std::cerr << "[VEX] TryFindMatchingIndex: invalid physical column id" << std::endl;
#endif
		return false;
	}
	...
	vex_compat::ForEachTableIndex(storage.GetDataTableInfo()->GetIndexes(), [&](Index &index) {
		if (!index.IsBound()) {
			auto &idx_type = index.GetIndexType();
#ifdef VEX_OPTIMIZER_DEBUG
			std::cerr << "[VEX] TryFindMatchingIndex: saw unbound index type=" << idx_type << std::endl;
#endif
			if (idx_type == GraphIndex::TYPE_NAME) {
				for (auto &idx_col : index.GetColumnIds()) {
#ifdef VEX_OPTIMIZER_DEBUG
					std::cerr << "[VEX]   unbound index col=" << idx_col << std::endl;
#endif
	...
	if (needs_bind) {
#ifdef VEX_OPTIMIZER_DEBUG
		std::cerr << "[VEX] TryFindMatchingIndex: binding index list" << std::endl;
#endif
	...
	vex_compat::ForEachTableIndex(storage.GetDataTableInfo()->GetIndexes(), [&](Index &index) {
		if (!index.IsBound()) {
#ifdef VEX_OPTIMIZER_DEBUG
			std::cerr << "[VEX] TryFindMatchingIndex: skipping still-unbound index type=" << index.GetIndexType() << std::endl;
#endif
			return false;
		}
#ifdef VEX_OPTIMIZER_DEBUG
		std::cerr << "[VEX] TryFindMatchingIndex: bound index type=" << index.GetIndexType()
		          << " name=" << index.GetIndexName() << std::endl;
#endif
		if (index.GetIndexType() == GraphIndex::TYPE_NAME) {
#ifdef VEX_OPTIMIZER_DEBUG
			std::cerr << "[VEX]   graph metric=" << vex::MetricName(index.Cast<GraphIndex>().GetMetric()) << std::endl;
#endif
			if (index.Cast<GraphIndex>().GetMetric() != query_metric) {
#ifdef VEX_OPTIMIZER_DEBUG
				std::cerr << "[VEX]   metric mismatch" << std::endl;
#endif
...
#ifdef VEX_OPTIMIZER_DEBUG
			for (idx_t i = 0; i < idx_col_ids.size(); i++) {
				std::cerr << "[VEX]   bound index col[" << i << "]=" << idx_col_ids[i] << std::endl;
			}
#endif
...
#ifdef VEX_OPTIMIZER_DEBUG
	std::cerr << "[VEX] FindGetChild: start type=" << LogicalOperatorToString(cur->type) << std::endl;
#endif
...
#ifdef VEX_OPTIMIZER_DEBUG
		std::cerr << "[VEX] FindGetChild: found PROJECTION, child type="
		          << LogicalOperatorToString(cur->children[0]->type) << std::endl;
#endif
...
#ifdef VEX_OPTIMIZER_DEBUG
		std::cerr << "[VEX] FindGetChild: found FILTER, child type="
		          << LogicalOperatorToString(cur->children[0]->type) << std::endl;
#endif
		cur = cur->children[0].get();
	}
...
static bool TryExtractEqualityFilter(const Expression &expr, TableIndex table_index,
                                     idx_t &out_col_index, Value &out_value);
static unique_ptr<vex::FilterPredicate> TryBuildFilterPredicate(
    const Expression &filter_expr, TableIndex table_index, GraphIndex &graph_idx,
    const vector<ColumnIndex> &column_ids);
...
#ifdef VEX_OPTIMIZER_DEBUG
		std::cerr << "[VEX] TryOptimizeANN: skipping because projection_ids.size="
		          << get->projection_ids.size() << std::endl;
#endif
		return false;
	}
...
#ifdef VEX_OPTIMIZER_DEBUG
	std::cerr << "[VEX] LimitOrderBy: found GET, table_index=" << DebugTableIndexValue(get_info.get->table_index)
	          << " has_proj=" << (get_info.proj != nullptr)
	          << " has_filter=" << (get_info.filter != nullptr) << std::endl;
...
static bool TryExtractEqualityFilter(const Expression &expr, TableIndex table_index,
	...
	std::cerr << "[VEX] TryExtractEqualityFilter: expr_type=" << (int)expr.GetExpressionType()
	          ...
	          << " table_index=" << DebugTableIndexValue(table_index) << std::endl;
#endif
