...
unique_ptr<GlobalSinkState> PhysicalVexCreateIndex::GetGlobalSinkState(ClientContext &context) const {
	...
	auto index_input = vex_compat::MakeCreateIndexInput(context, TableIOManager::Get(table.GetStorage()),
	                                                    table.GetStorage().db, info->constraint_type,
	                                                    info->index_name, storage_ids, unbound_expressions,
...
unique_ptr<LocalSinkState> PhysicalVexCreateIndex::GetLocalSinkState(ExecutionContext &context) const {
	...
	if (info->index_type != GraphIndex::TYPE_NAME) {
		...
		IndexStorageInfo storage_info;
		auto index_input = vex_compat::MakeCreateIndexInput(context.client, TableIOManager::Get(table.GetStorage()),
		                                                    table.GetStorage().db, info->constraint_type,
		                                                    info->index_name, storage_ids, unbound_expressions,
		                                                    storage_info, info->options);
...
#else
SourceResultType PhysicalVexCreateIndex::GetData(ExecutionContext &context, DataChunk &chunk,
                                                 OperatorSourceInput &input) const {
#endif
