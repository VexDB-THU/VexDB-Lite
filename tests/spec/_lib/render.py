...
RENDER_MARKER = ".vexdb-render-output"
RENDER_MARKER_CONTENT = "Generated by tests/spec/_lib/render.py; safe to replace.\n"


def prepare_engine_output(out_base: Path, engine: str) -> Path:
    """Create a renderer-owned output directory without claiming user data."""
    eng_out = out_base / engine
    marker = eng_out / RENDER_MARKER

    if eng_out.is_symlink():
        raise RuntimeError(f"refusing to use symlink output directory: {eng_out}")

    if eng_out.exists():
        if not eng_out.is_dir():
            raise RuntimeError(f"output path is not a directory: {eng_out}")
        if marker.is_file():
            if marker.is_symlink() or marker.read_text(encoding="utf-8") != RENDER_MARKER_CONTENT:
                raise RuntimeError(f"output ownership marker is invalid: {marker}")
            if eng_out.resolve().parent != out_base.resolve():
                raise RuntimeError(f"output directory escapes the requested root: {eng_out}")
            shutil.rmtree(eng_out)
            eng_out.mkdir(parents=True)
        elif any(eng_out.iterdir()):
            raise RuntimeError(
                f"refusing to overwrite non-empty directory not owned by the renderer: {eng_out}"
            )
    else:
        eng_out.mkdir(parents=True)

    marker.write_text(RENDER_MARKER_CONTENT, encoding="utf-8")
    return eng_out
...
def _connection_suffix(step: dict) -> str:
    """Render an optional DuckDB sqllogictest connection name safely."""
    connection = step.get("connection", "")
    if not connection:
        return ""
    if not isinstance(connection, str) or not re.fullmatch(r"[A-Za-z_][A-Za-z0-9_]*", connection):
        raise ValueError(f"invalid sqllogictest connection name: {connection!r}")
    return f" {connection}"
    ...
    for step in spec.get("steps", []):
        ...
        if "statement" in step and "expect_error" not in step:
            for stmt in split_sql(render_template(step["statement"], dialect)):
                lines += ["statement ok" + _connection_suffix(step), stmt, ""]
        elif "expect_error" in step:
            sql = step.get("query") or step.get("statement", "")
            lines += [
                "statement error" + _connection_suffix(step),
            ...
            lines += [
                _query_header(field_types, step.get("_sort", "")) + _connection_suffix(step),
...
def main() -> int:
    ...
    total = 0
    for eng in engines:
        eng_out = prepare_engine_output(out_base, eng)
