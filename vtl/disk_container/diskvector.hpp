...
public:
    ...
    template <AccessorLockType lock_type> void set_n(size_t idx, size_t n, const T *elem)
    {
        ...
        for (size_t i = 0; i < n; i += write_amount) {
            ...
            pd.mark_dirty();
            if (_need_wal) {
                dv_log.xl_add_elem(pd, (char *)this->_offset_by(elem, i),
                ...
                if (partial_data != NULL) {
                    ...
                    if (_vector._need_wal) {
                        _vector.dv_log.xl_set_elem_partial(pd, partial_data, partial_len,
                            _vector._get_offset(offset) + (partial_data - (char *)_vector._get(data, offset)));
                    }
                }
                _vector._blkmgr.template unlock_page_data_custom<lock_type>(pd);
                _vector._blkmgr.release_page(pd);
                    ...
                    for (size_t i = 0; i < batch_size; ++i) {
                        ...
                        if (_apply_func(elem)) {
                            ...
                            if (_vector._need_wal) {
                                ...
                                if (_partial_size > 0) {
                                    _vector.dv_log.xl_set_elem_partial(pd, (char *)elem, _partial_size, elem_offset);
                                } else {
                                    _vector.dv_log.xl_add_elem(pd, (char *)elem, _vector.data_size(), elem_offset);
                                }
                            }
                        }
                    }
